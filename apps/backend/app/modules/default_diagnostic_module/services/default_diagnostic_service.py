...
from ..schemas import DefaultDiagnosticCreate


DEFAULT_DIAGNOSTIC_CODE = "default"
...
async def create_default_diagnostic(
    ...
    try:
        ...
        run = DiagnosticRun(
            ...
            diagnostic_code=DEFAULT_DIAGNOSTIC_CODE,
        ...
        default_diagnostic = DefaultDiagnostic(diagnostic_run_id=run.id)
        session.add(default_diagnostic)
        ...
        DBErrorHandler.handle(err=err, model=DefaultDiagnostic, action="creating")

    stmt = (
        select(DefaultDiagnostic)
        .options(selectinload(DefaultDiagnostic.diagnostic_run))
        .where(DefaultDiagnostic.id == default_diagnostic.id)
    )
