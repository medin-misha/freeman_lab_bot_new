...
`app.modules.telegram` is the reusable Telegram integration module for the FastAPI application.
...
`telegram` is a feature module built on top of `system`.
...
- If you change exports, update `telegram/__init__.py`.
...
2. If it is shared infrastructure, edit `system` instead of `telegram`.
