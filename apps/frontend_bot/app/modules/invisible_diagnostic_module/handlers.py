"""
Хендлеры invisible diagnostic модуля.

Модуль повторяет flow базовой диагностики, но использует собственные тексты,
PDF-инструкцию и diagnostic type для backend.
"""
...
class InvisibleDiagnosticStates(StatesGroup):
    """Изолированные FSM-состояния сценария диагностики невидимости."""
...
async def send_invisible_diagnostic_preview(
    ...
    """Отправляет preview диагностики невидимости и PDF-инструкцию."""

    await state.clear()
    await state.set_state(InvisibleDiagnosticStates.waiting_for_file)
    await message.answer_document(
        document=FSInputFile(INVISIBLE_DIAGNOSTIC_PREVIEW_FILE_PATH),
        caption=_MESSAGES["diagnostic_preview"],
        reply_markup=get_invisible_diagnostic_reply_keyboard(),
    )
...
async def receive_invisible_diagnostic_media(
    ...
    if description is not None:
        await _submit_invisible_diagnostic(
            message=message,
            state=state,
            payload=payload,
    ...
    await state.set_state(InvisibleDiagnosticStates.waiting_for_text)
    await message.answer(
...
async def _submit_invisible_diagnostic(
    *,
    message: Message,
    state: FSMContext,
    ...
    await state.set_state(InvisibleDiagnosticStates.sending_to_server)
    ...
    try:
        await create_diagnostic_from_telegram_file(
            ...
            description=description,
            diagnostic_type=INVISIBLE_DIAGNOSTIC_TYPE,
        )
    except Exception:
        logger.exception(
            "Invisible diagnostic creation failed for chat_id=%s",
            message.chat.id,
        )
        await state.clear()
        await message.answer(
