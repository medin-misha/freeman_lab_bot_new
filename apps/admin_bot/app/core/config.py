...
class MainSettings(BaseSettings):
    ...
    @property
    def admins_chat_ids(self) -> list[int]:
        """Возвращает chat id админов из CSV env-переменной."""

        chat_ids: list[int] = []
        for raw_item in self.admins_chat_ids_raw.split(","):
            item = raw_item.strip()
            if not item:
                continue
